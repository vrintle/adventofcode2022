...
void solve() {
	...
	debug(a);
	ll n = sz(a), m = sz(a[0]);
	vvl rp(n, vl(m + 1, -1)), rs(n, vl(m + 1, -1)), cp(m, vl(n + 1, -1)), cs(m, vl(n + 1, -1));
	F0(n, i) {
		F0(m, j) {
			rp[i][j + 1] = max(rp[i][j], (ll)a[i][j] - '0');
			cp[j][i + 1] = max(cp[j][i], (ll)a[i][j] - '0');
		}
	}
	for(ll i = n - 1; i >= 0; i--) {
		for(ll j = m - 1; j >= 0; j--) {
			rs[i][j] = max(rs[i][j + 1], (ll)a[i][j] - '0');
			cs[j][i] = max(cs[j][i + 1], (ll)a[i][j] - '0');
		}
	}
	debug(rs, cs);
	ll ans = 0;
	F0(n, i) {
		F0(m, j) {
			if(a[i][j] - '0' > min({rp[i][j], cp[j][i], rs[i][j + 1], cs[j][i + 1]})) {
				debug(i, j);
				ans++;
			}
		}
